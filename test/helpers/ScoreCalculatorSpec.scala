...
class ScoreCalculatorSpec extends Specification with Mockito {
  ...
  "ScoreCalculator - basic scores" should {
    "Score full points the first time" in {
      ScoreCalculator.getScoreForThisPunch(1, RedBall) === RedBall.getPointsValue
    }

    "Score 3/4 points the second time" in {
      ScoreCalculator.getScoreForThisPunch(2, BlueBall) === (BlueBall.getPointsValue * 0.75)
    }

    "Score 1/2 points the third time" in {
      ScoreCalculator.getScoreForThisPunch(3, PinkBall) === (PinkBall.getPointsValue * 0.5)
    }

    "Score 1/4 points the forth time" in {
      ScoreCalculator.getScoreForThisPunch(4, RedBall) === (RedBall.getPointsValue * 0.25)
    }

    "Score no points the fith time" in {
      ScoreCalculator.getScoreForThisPunch(5, RedBall) === 0
    }

    "Score no points the 20th time" in {
      ScoreCalculator.getScoreForThisPunch(20, RedBall) === 0
    }
  }

  "ScoreCalculator - Red then another ball" should {
    "Score 10 for Red - another Red" in {
      val testData = PunchedControls(2, List[ControlCode](RED_1, RED_2))
      val res = ScoreCalculator.calcScoresForAllPunches(testData)
  ...
  "ScoreCalculator - Red, colour, red, different colour" should {
  ...
  "ScoreCalculator - Red, colour, red, same colour" should {
  ...
  "ScoreCalculator - All ctrls punched" should {
  ...
  "ScoreCalculator - Red, colour, same red, any colour" should {
  ...
  "ScoreCalculator - countHowManyValidPunchesOfThisCtrl" should {
    "Return 1 if control not in the list" in {
      ...
      val res = ScoreCalculator.countHowManyValidPunchesOfThisCtrl(testData, RED_3)
      res === 1
    }
    ...
    "Return 2 if control is in the list once" in {
      ...
      val res = ScoreCalculator.countHowManyValidPunchesOfThisCtrl(testData, RED_2)
      res === 2
    }
