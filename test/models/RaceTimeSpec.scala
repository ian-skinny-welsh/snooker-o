...
class RaceTimeSpec extends Specification with Mockito {
  ...
  "ValidRaceTime" should {
    "Instanciate from two Excel strings 1" in {
      val res: Option[RaceTime] = fromString("00:01:00", "00:21:00")
      res.isDefined === true
      res.get.isInstanceOf[ValidRaceTime] === true
      res.get.timeTaken === TimeValue(0, 20, 0)
    }

    "Fail to instanciate from two Excel strings 2" in {
      val res: Option[RaceTime] = fromString("0:01:00", "00:21:00")
      res === None
    }

    "Fail to instanciate from two Excel strings 3" in {
      val res: Option[RaceTime] = fromString("00:01:00", "00:1:00")
      res === None
    }

    "Fail to instanciate from two Excel strings 4" in {
      val res: Option[RaceTime] = fromString("00:0:00", "00:2:00")
      res === None
    }
  }
}
